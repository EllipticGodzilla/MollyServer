...
public class MollySettingsPanel implements SettingsComponent {
    ...
    public static void init() {
        ...
        add_item("server/network settings", new NetworkSettings(), NetworkSettings.ok_listener, NetworkSettings.apply_listener);
    }
    ...
    public static boolean add_item(String path, SettingsComponent component, ActionListener ok_listener, ActionListener apply_listener) {
        ...
        if (root_menu_len == -1) {
            Logger.log("impossibile aggiungere CascateItem al Settings component, specificare un root menu per: " + path, true);
            return false;
        }
        else {
